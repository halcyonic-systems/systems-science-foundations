...
The formalization runs to roughly 10,800 lines of Lean 4 with zero unresolved proof obligations and no custom axioms. The construction is easier to picture than it sounds. Draw each tradition as a small diagram of what-depends-on-what (a "shape category": positions joined by arrows that encode the tradition's structural commitments). The convergence is the claim that the same two-dot arrow sits inside every one of those diagrams without collapsing distinct things, formally a functor from the walking arrow into each shape that is injective on objects. Object-injectivity is machine-checked for all eight embeddings. Faithfulness follows from every hom-set in the core being a singleton, an argument the source records in comments rather than as a theorem. Maximality of the core is developed in the companion document and is not machine-checked.
