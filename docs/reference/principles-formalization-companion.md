...
**Next**: Only **#6 Evolution** remains unformalized (11/12 done). **Two stretches open**: (a) Self-Models — existence/obstruction of a *faithful* self-model via Lawvere's fixed-point theorem (Mathlib has only the powerset diagonal); (b) Information — mutual information, the data-processing inequality, operational channel capacity, and the semantic/viability layer above the genus. Both are separate research-level sessions. **#6 is the closing research push**: blind variation + environmental selection (the agential layer #11/#12 is now complete, so #6 finishes the 12 and unlocks the full dependency-DAG deliverable). `Selection.lean` holds Bunge's static selective-action (adapted/eliminated sets) — a partial substrate, not fitness/variation dynamics.
