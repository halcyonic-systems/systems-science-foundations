...
**Next**: Remaining unresolved: #6 Evolution, #12 Improvability. **Two stretches open**: (a) Self-Models — existence/obstruction of a *faithful* self-model via Lawvere's fixed-point theorem (Mathlib has only the powerset diagonal); (b) Information — mutual information, the data-processing inequality, operational channel capacity, and the semantic/viability layer above the genus. Both are separate research-level sessions. **#11 → #12 note**: Improvability is now the only Tier-3 holdout, and the roadmap already flags it as depending on #11 — directed design needs a model (#9) *and* understanding (#11) plus an external goal; the #11 finding (understanding is a real, independent constraint) tightens the case that #12 is independent too.
