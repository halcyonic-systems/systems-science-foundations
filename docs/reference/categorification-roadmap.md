...
**✓ RESOLVED — tractable core (2026-07-10, `Systems/Joslyn/Control.lean`).** Layered shape, frozen from the 1995 text in a sketch session: `Control₁` (Def 25, envelope form with the §3.3.1/§3.3.2 dissociation/overconstraint guards), `Control₂` (Def 28 — a nonempty region surviving inside every induced envelope; "constrained₂" = reduced second-order variety, not eliminated), and `Control₂Realized` (Prop 29's decomposition as data: disturb h, effect g, `compensates`). Coherence theorems: `cancellation` (the "variation of C and O_E cancel" line, axiom-free), `toControl₂` (the realization is control₂ on the controlled variables — the Marken face, not E × I), `toControl₁` (Prop 29's inner claim, axiom-free). **Governance bridge**: `Homeostat.toControl₂Realized` — every perfect (one-tick) regulator realizes control₂, machine-checking "the Joslyn formalization IS the Governance formalization"; asymptotic version deferred (needs dynamics). **DEFERRED on Mesarovic 1964 p.14**: the existence direction of Prop 29 (every control₂ admits a decomposition) — same posture as Lawvere for #10. *Update 2026-07-11: the source is acquired and its two cores are formalized (`Systems/Mesarovic/Decomposition.lean`, fully axiom-free: `Set.peel` constructive triadic decomposition + `binaryJoin_parityRel_ne` dyadic floor; transcription + analysis in `docs/reference/mesarovic-1964-decomposition.md`). Same day, part 2's iff modernized and the existence direction RESOLVED: `middleFactorizable_iff_eq_middleJoin` (factorizable by ANY dyadic pair ↔ R = join of its own shadows — closes the printed proof's "any splitting" gap) + parity refusing all three pivots; and Prop 29 existence is **degenerate at the set tier** — trivially true in stable-region form (`Control₂.exists_realization` via the ignore-the-disturbance `trivialRealization`), false in envelope-faithful form (`toControl₂_envelope_const`: every realization's outer envelopes are constant). The semantic content (effector VARIATION cancelling) is inexpressible in set-tier data — the rule-vs-law gap class, but exhibited by a theorem pair instead of narrated. Faithful Prop 29 existence = a dynamical-tier target.* **Foundational finding (pairs with §4.1's)**: the initial `Classical.choice` pulls were proven INCIDENTAL — `Set.singleton_ne_univ`, `Eq.ge`-via-order-instance, and `Set.biUnion_singleton` are classical in Mathlib but have elementary axiom-free replacements (`singleton_ne_univ'`, membership transport, two-inclusion proof); all five results are choice-free. With §4.1's LEM-essential `compl_constraint`, both directions of the essential-vs-incidental question are now demonstrated in this repo.
...
2. **Convergence, timescale**: Joslyn Def 28's "hierarchically distinct level, slower time scale" ≡ `TimescaleDecomposition`, machine-checked: packaging the HCGS as coupled dynamics (`toCoupled`), the frozen-coordinator fast dynamics IS the level homeostat's feedback law (`fast_ops_is_level_homeostat = rfl`). **Calibrated statement of what this shows**: under independent formalization into this common set-theoretic ontology, Joslyn's control₂ hierarchy and the minimal HCGS are not merely isomorphic — they are definitionally equal up to a provably lossless repackaging (`toCoupled_combinedLaw` is the coherence receipt that the packaging forgets and adds nothing). The claim is about the two SSF formalizations — each built against its source text, in different files, without aiming at the other — not about the prose theories directly; and it holds for the two-level HCGS under run 4's perfect-regulator hypothesis. Still stronger than the usual convergence tier: most such claims top out at "isomorphic via a constructed bridge"; this one collapsed past the bridge entirely.
