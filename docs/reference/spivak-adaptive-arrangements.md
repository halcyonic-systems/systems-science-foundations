...
1. **DONE at shape level (2026-07-02); classifier DONE at data level (2026-07-06).** Encode the 0-ary-morphism definition of system and the open/closed classifier (Def 5.3.2 vocabulary) over a minimal operad structure; show the kernel functor **2** → (system-arrows) — the "system = morphism" verification. *Shipped as `ShapeSpivak.lean` (4 positions, 5 arrows, the drive cycle) + `klirToSpivak` in CommonCore. The Def 5.3.2 vocabulary now lives in `SpivakSystem.lean` as classifier Props (PotentialFree/Static/Stateless/Driven) with `potentialFree_static` as the bite; the operad-native encoding remains follow-up.*
2. **Ladder DONE at shape level (2026-07-02); data-level view DONE (2026-07-06).** Encode the abstract adaptive-arrangement datum (Def 4.1.1) and the commitments ladder (Remark 4.1.2) as named preconditions over the kernel — the entry-#8 gate. *`myersToSpivak : I_Myers ⥤ I_Spivak` machine-checks the ladder claim (Spivak = Myers + potential + drive). `SpivakSystem.lean` (CLS24 Def 7 shape, factorization law) carries `Kernel.toSpivak` with round trip, faithfulness, and the cost-as-stratum theorems: every kernel-generated view is potential-free and static — the value channel is data the kernel cannot supply, not a condition it might satisfy. The abstract Def 4.1.1 datum (𝓜, 𝓠, J, R) over mathlib CT remains follow-up: mathlib has monoidal cats/functors, monads, Kleisli, but NO `MonoidalMonad` (verified 2026-07-06) — R is ours to define, and the Kernel↔categorical bridge is undesigned.*
