...
2. **Ladder DONE at shape level (2026-07-02).** Encode the abstract adaptive-arrangement datum (Def 4.1.1) and the commitments ladder (Remark 4.1.2) as named preconditions over the kernel — the entry-#8 gate. *`myersToSpivak : I_Myers ⥤ I_Spivak` machine-checks the ladder claim (Spivak = Myers + potential + drive). The Def 4.1.1 datum (𝓜, 𝓠, J, R) as named preconditions remains data-level follow-up. CLS24 (arXiv:2404.16140) read 2026-07-02 — vault deep-read at `operations/sessions/2026-07-02/references/cls24-open-energy-driven-systems.md`; key finding: the value commitment enters as a monadic effect (OpenErg = Kleisli of (−)×ℝ), so R in the datum is where the entry-#8 commitment lives.*
