...
\author{Claude (Fable 5.1), as prompted by Shingai Thornton\thanks{Byline per the Multivar repository's convention. The encoding decision, the readings of \J, and \S4 are Shingai Thornton's; \S2 is machine-checked in \texttt{systems-science-foundations/Systems/Category/\{ShapeJoslyn, CyclicObstruction, JoslynIncomparability\}.lean}, checkout \texttt{4288793}. \J{} = C.~Joslyn, ``Semantic Control Systems,'' \emph{World Futures} 45 (1995) 87--123.}}
...
This note does one thing to (30): it records the three named relations of Figure~1 as the arrows of a quiver and takes the free category on it (\S1). A theorem follows from the loop alone (\S2). \S3 says what the theorem does not claim. \S4 asks whether the drawing is faithful to what you meant. \S5 is a separate provenance item.
...
Every other tradition encoded in the same program (Klir, Bunge, Mobus, Myers, Wymore, Mesarovi\'c) has an acyclic $Q$, hence finite hom-sets, and is placed by faithful comparison functors. $Q_J$ is the first cyclic shape in the landscape; the only other is Spivak's value loop, which instantiates the same lemma. Feedback is where the finite-shape method stops; Theorem~\ref{thm:main} makes that folklore a theorem whose sole premise is the cycle.
\end{remark}
