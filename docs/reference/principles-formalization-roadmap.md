...
| 4 | Dynamics | **Complete** (structural — quantitative convergence deferred) **+ life-cycle closure 7/25** (Lifecycle.lean, SSF #30 — dynamics whose carrier is the 8-tuple, not a state space: `wellFormed_of_reaches` closes the paper's §4; `WellFormed`/`PreTuple` reify the coherence constraints with two-way adequacy; `additive_components_monotone` proves Mobus's ∪ cannot reach Decline/Dissolution, `growthStep_additive` recovers ∪ as the Growth regime; `step_not_additive` witnesses non-vacuity. Open: interface removal, Dissolution-as-terminal, `F_φ` in Veliov's terms) **+ H-instantiation 8/14** (CircuitHistory.lean, research-tier, bert-lenses#112 §4 — `recordedHistory_is_coalgebra_prefix` at BERT's abstracted carrier; identity observer load-bearing; `propext`+`Quot.sound` only. Open: the total/spark carrier asymmetry as a proposal amendment; connecting `f` to a verified stepper abstraction) | Dynamical systems, equilibrium, flows, timescale decomposition; admissible-successor relations (Veliov discrete inclusions); final coalgebras (hand-built, Mealy-shaped) | — | #3 | 2 |
...
| 9 | Internal Models | **Theorem** (InternalModel.lean — simulation lifts to all horizons; Rosen fast model `AnticipatoryModel` races `n·lead` ahead, lockstep = lead 1; → Conant-Ashby) | Simulation relation, semiconjugacy, induction | — | #4, #8 | 2 |
| 10 | Self-Models | **Theorem** (tractable core — SelfModel.lean; diagonal of #9, self-anticipation, accurate set invariant; fast diagonal `FastSelfModel`: accurate fast self-models force orbit periodicity, invariance is a lockstep privilege; faithful-existence/Lawvere deferred) | Fixed-point theory, self-reference | — | #9 | 3 |
...
Theorem-tier, no new axiom. **Deferred (research-level)**: mutual information, the data-processing inequality, operational channel capacity (sup of mutual information over input distributions), and the **semantic / viability layer** above the genus — meaning and goal-relevance, refining the genus by a set-point à la Governance #8. The top of the stack is left deliberately open per the long-term aim of a conception of information not constrained by Shannon's communication statistics.
  ...
  **Research thread (parked, vault-side) — candidate spine for this deferred semantic layer.** Channel-theoretic semantic information is the candidate formalism: meaning as the satisfaction relation `token ⊨ type` (Barwise–Seligman classifications + infomorphisms), with truthfulness as soundness (Floridi) and emergence as gluing-obstruction (Abramsky contextuality) — meaning carried in the structure, not a measure bolted on. Klir's Generalized Information Theory was evaluated and **rejected** for this role (epistemic/constructivist, against the project's perspectival realism; it's a measurement toolbox, not a foundation). Docs (5; hub = `semantic-information-k2-bridge.md`, whose top "Status — PARKED (resume here)" block is the entry point): `semantic-information-theory-primer.md` (systems-language primer), `semantic-information-reading-order.md` (CrossRef-verified citations; Zotero collection "Semantic Information Theory (K≅2 thread)", key PVA7MKC8, 3 OA PDFs), `semantic-information-k2-bridge.md`, `semantic-information-bunge-experiment.md`, `semantic-information-infomorphism-check.md`.

  **Two first-cut tests run (2026-06-15), both PASSED — the bridge is confirmed and sharpened, not proven:** (1) a K≅2 lens is a full *local logic*, not just constraints — Mobus irreflexivity (`∀`) is a law (constraint layer), the Bunge bond (`∃`) is an existence/support condition (classification layer); the quantifier carries the engineering-vs-ontology distinction. (2) The view-generation maps ARE genuine infomorphisms — `ViewGeneration.lean`'s `edges := {e | e.toPair ∈ k.dep}` literally *is* the satisfaction biconditional, so "view = classification linked to the kernel by an infomorphism" is true, not analogy. (3) The K≅2 invariant is the shared **reduct** (token-coarse/type-poor; views map down to it — home of convergence/realism), which is **dual** to a Barwise–Seligman channel **core** (token-fine/type-rich — home of flow/emergence/Abramsky gluing); they are NOT the same universal object. **Resume:** (a) name the reduct/refinement dual in **Cla**/Chu; (b) then `lean-formalize` the infomorphism laws + the law/existence species split (scaffold in the bunge-experiment doc); (c) wire as the formalization of this semantic layer. Low priority, PhD-flavored.

  **Closure target (external anchor, issue #15, added 2026-07-07)** — Shadab, Cody, Salado & Beling 2024 (IEEE OJSE 2, DOI 10.1109/OJSE.2024.3369070) formalize *functional closure* (behavior depends only on a minimal input–output set; a condition on the relation) and *informational closure* (`I(S_{n+1}; E^O_n | S_n) = 0`; a predicate over state trajectories and measures), with functional as the zero-mutual-information special case of informational (their Theorem 1). Targets: a `FunctionalClosure` predicate in `Systems/Core` (distinct from the closure-under-composition sense in `Systemness.lean`), their context-system construct (S⁰/S^C/E^I/E^O) as a Bunge-view extension, and machine-checking their Eq. 6 relaxation + Theorem 1 entropy chain — the Theorem 1 check is a natural first consumer of this deferred mutual-information layer. Their two closures split exactly along the condition-vs-data mode dichotomy (independent replication, different lineage). Downstream consumer: bert#106 (open/closed classifier semantics). Vault synthesis: `strategy/research/se4ai-closure-synthesis.md`.
...
**✓ ANTICIPATORY VERSION RESOLVED (2026-07-10, same file).** The Rosen fast-model — a model running FASTER than its system — is theorem-tier too. `AnticipatoryModel R S` replaces the one-step square with `model (internalDyn r) = systemDyn^[lead] (model r)` (one internal tick = `lead` system steps); `AnticipatoryModel.tracks` proves the lead compounds (`model (internalDyn^[n] r) = systemDyn^[n * lead] (model r)`) by the same induction skeleton plus `Function.iterate_add_apply`/`Nat.succ_mul`. Round-trip `InternalModel.toAnticipatory`: lockstep is exactly `lead = 1`, so the anticipatory model is a strict generalization. Choice-free (now the #9 headline in `scripts/axiom-profile.sh`). Remaining future work here: stochastic/approximate models only.
...
**✓ FAST DIAGONAL RESOLVED (2026-07-10, same file).** `FastSelfModel` — the diagonal of `AnticipatoryModel` (#9's Rosen extension): the self-model gains `lead` steps of the system's OWN dynamics per tick, with `toAnticipatory` transferring `tracks` (`selfModel (dyn^[n] s) = dyn^[n·lead] (selfModel s)`). The diagonal's own content is the **accuracy/speed tension**, invisible at lockstep: (1) `accurate_collapse` — accuracy at both ends of an orbit segment collapses the fast clock onto the slow one (`dyn^[n·lead] s = dyn^[n] s`); (2) `accurate_forces_periodic` — the reached state is periodic with period dividing `n·(lead−1)`: **perfect self-anticipation collapses time**, a Lawvere-flavoured obstruction from bare iterate arithmetic; (3) `accurate_invariant` is a `lead = 1` privilege — refuted at `lead = 2` by the doubling witness (`ℕ`, successor, doubling: accurate at 0, wrong at 1). All choice-free; `accurate_forces_periodic` is the new #10 headline in `scripts/axiom-profile.sh`. Remaining future work: faithful-existence/Lawvere (unchanged), stochastic models.
...
**Open question** *(RESOLVED 6/09 — `Understanding.lean`)*: Is Understandability independent of #9? **Yes.** Path #2 (compression) closed it. `Understanding S M` = an onto, **lossy**, **non-degenerate** coarse-graining `abstract : S → M` (`compresses : ¬ Injective`, `nontrivial : Nontrivial M`) whose dynamics commute one step; from that, `tracks`/`predict_correct` lift one-step correctness to all horizons, and on finite carriers `card_lt`/`hartley_lt` give `card M < card S` and a strict Hartley drop — the quantitative bound (path's optional deepening) came for free, and it *is* the #7 Hartley nonspecificity drop. **The `nontrivial` field is a finding:** without it the total collapse `S → Unit` (onto, lossy, vacuously commuting) would "understand" every ≥2-state system — a model distinguishing nothing predicts nothing — so non-degeneracy is required to make the principle non-vacuous. **Independence (#9 ⇏ #11) by two witnesses:** `InternalModel.refl` makes every system model itself for free, but (i) a one-state system admits no understanding (`modeling_does_not_imply_understanding` — nothing to compress) and, more strikingly, (ii) the **3-cycle on `Fin 3`** has a model yet cannot be understood (`cyclic3_no_understanding`) — 3 is prime, so the only dynamics-respecting identification is the excluded total collapse: *understanding is impossible even with room to compress; the dynamics forbid it.* The compression axiom ("models must be simpler than the modelled") is the hidden epistemological content Mobus's "corollary" obscured; if anything the arrow reverses (understanding ⟹ modelling). #9/#11 are *dual* (decode `R→S` vs encode `S→M`, Rosen). **This is the genuine finding the section predicted.** Paths #1 (Rosen square w/ explicit decode section) and #3 (faithful functor from a simpler shape category) remain optional deepenings, not gaps.

**Effort**: Done — core was LOW (dual of #9's machinery); the dynamics-incompressible witness was the one piece needing real work (a congruence-closure argument on `Fin 3`, closed by `decide`-backed arithmetic). **Risk realized as high-reward**: #11 is an independent axiom, a contribution to the axiomatization (the principle count does *not* reduce here — it stays honest at one more axiom than the "corollary" reading suggested).
