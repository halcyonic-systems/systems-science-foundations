...
Theorem-tier, no new axiom. **Deferred (research-level)**: mutual information, the data-processing inequality, operational channel capacity (sup of mutual information over input distributions), and the **semantic / viability layer** above the genus — meaning and goal-relevance, refining the genus by a set-point à la Governance #8. The top of the stack is left deliberately open per the long-term aim of a conception of information not constrained by Shannon's communication statistics.
  ...
  **Research thread (parked, vault-side) — candidate spine for this deferred semantic layer.** Channel-theoretic semantic information is the candidate formalism: meaning as the satisfaction relation `token ⊨ type` (Barwise–Seligman classifications + infomorphisms), with truthfulness as soundness (Floridi) and emergence as gluing-obstruction (Abramsky contextuality) — meaning carried in the structure, not a measure bolted on. Klir's Generalized Information Theory was evaluated and **rejected** for this role (epistemic/constructivist, against the project's perspectival realism; it's a measurement toolbox, not a foundation). Docs: `strategy/research/semantic-information-theory-primer.md` (systems-language primer), `semantic-information-reading-order.md` (CrossRef-verified citations; Zotero collection "Semantic Information Theory (K≅2 thread)", key PVA7MKC8, 3 OA PDFs attached), `semantic-information-k2-bridge.md`. **The bridge proposes a decisive cheap test tied to view-generation** (the Bunge-bond / Mobus-irreflexivity costs in the prerequisite line above): express the Bunge view as a classification + local logic and check whether the bond appears as a constraint that *fails to lift* to the channel's global logic. If so, a K≅2 lens ≈ a local logic, and channel theory is the information-theoretic face of K≅2 — a new proof technique for the remaining views. Low priority; one worked Bunge example settles it.
