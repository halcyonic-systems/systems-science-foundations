...
## Addendum: The base quiver and constraint packages (2026-08-25)

An interpretive frame, recorded because it reorganizes what the encodings already prove. It adds no new theorem; every load-bearing sentence below names the result it leans on.

**The substrate the traditions share is a quiver, not a graph and not a category.** Not a graph: the systems edge is directional by assertion — an arrow says one position needs another — where graph theory's default edge is symmetric connectivity. Not a category: composition is precisely what the traditions do not jointly assert (`free_category_maximality_fails`; the repaired maximality statement is quiver-level). The substrate is the quiver: typed nodes, asserted directed edges, nothing else paid for.

**What individuates a tradition is its constraint package over that substrate.** The slots usually described as "what a definition includes" — typed flows, boundary, environment, history, timescale, mechanism — are exactly where the encodings diverge: the 8-tuple slots that are `Unit` in one encoding and load-bearing in another, Joslyn's out-degree bound, Willems's no-composable-pair, Bertalanffy-68's everything-bound. Read this way the object counts are a measurement: `ShapeKlir` (2 objects) → `ShapeBunge` (3) → `ShapeMobus` (8) is a gradient of ontological commitment, and the forgetful functors down the chain strip commitments one package at a time (the study-notes observation "read backwards, it's a completion," now said forwards).

**Consequence for the two lanes.** Klir's floor is what remains when every ontological commitment is stripped — an epistemic structure needing no claim about what systems are, which is why a constructivist could stop there and why Mac Lane could file the field under applied mathematics. Bunge and Mobus are doing the other job: saying what the formalism is *of*. The kernel hosts both lanes without arbitrating, because the shared shape and the per-tradition commitments are separated by construction.

**Limits of the frame, stated.** "Constraint package" is informal: the encoded divergences are of mixed kind (extra objects, degree bounds, forbidden patterns, qualifying conditions), and at least one demand — Bertalanffy's difference-making clause, Bunge's nonempty bondage — lives below the shape layer entirely (see the 2026-08-05 addendum). A fibration-shaped formalization (traditions as objects over the base quiver) is a candidate for the categorification roadmap, not a result.

## Addendum: Rosen 1978 — the eighth tradition is the core itself (2026-09-14)

Rosen, *Fundamentals of Measurement and Representation of Natural Systems* (1978), Definition 2.9.1: "A system (or formal system) shall consist of a pair (S, F), where S is a set and F is a family of real-valued mappings defined on S" (book p. 54; atlas entry 010, human-verified against the page 2026-09-14). `ShapeRosen`: positions `states`, `observables`; one arrow `defined_on : observables → states`.

**The encoding decision, stated before the result.** ℝ is the substrate, not a position: Proposition 2 (p. 26) fixes every observable's codomain as the reals once and for all, so ℝ is ambient the way `Set` is for Klir. On that reading the quiver has two objects and one arrow. On the other defensible reading — ℝ as a third position with a second arrow F → ℝ — the shape is a cospan and what follows fails on object count. Both are recorded in the module docstring; only the first is built.

**Result.** `rosenKlirEquiv : Paths RosenPosition ≌ Paths KlirPosition` (`RosenKlirIso.lean`): comparison functors both ways via `Paths.lift`, object-injective, faithful, and **both round trips equal to the identity functor** (`Paths.ext_functor`). An isomorphism of the free categories, not a faithful embedding one way. Axioms propext, choice, Quot.sound; thinness (`rosen_path_subsingleton`) axiom-free. `lake build` green (2283 jobs), zero sorry.

**What it is worth.** Second tradition after Bertalanffy 1968 whose *entire* quiver is the core, and the first from outside the Bertalanffy–Klir lineage: Rosen built modelling inside the arrow category (§7.10, the functor category over the one-arrow diagram) without claiming the arrow is what definitions of system share. The convergence claim stays ours; the universe was his.

**What it does not say.** Shape identity is not semantic identity, as with Willems/Mesarović. The forced assignment `things ↦ states` reads the kernel's relata as the states of *one* system, and Rosen's F is a family of functions out of them, not a relation among parts. Mapping 008 (atlas) names that as the cost "states stand in for things" and owes a Bunge witness that the bond is not recoverable from (S, F). **Obstruction audit:** Q_Rosen is Q_Klir up to renaming, so it admits nothing Q_Klir does not; no separate degree lemma is needed, the isomorphism is the audit. **No `CommonCore` embedding row**, for the reason given for Bertalanffy: an isomorphism is not an embedding into a larger shape.

### Addendum to the Rosen entry: view generation, the bond, conjugacy (2026-09-16)

Mapping 008's remaining three claims, each built from the `.lean` and the page images (scan pages 202–204 = book pp. 184–186 for §7.10), never from the memo.

**Claim 2 — `Kernel.toRosen` (`Systems/Klir/ViewGeneration.lean`).** Data-level `RosenSystem α` = `states : Set α`, `observables : Set (α → ℝ)`, and `defined_on` (an observable vanishes off S — "defined on S" as a Prop, the walking arrow materialised exactly as `Kernel.dep_on` does for Klir). `RosenSystem.indist` is Rosen's own R_F (p. 54, "a set of reduced states S/R_F"); `RosenSystem.toKlir` projects (S, R_F). The generated view has one observable per dependency class (`classIndicator`, values in {0, 1} ⊆ ℝ — the canonical witness, as `PUnit` is for Mobus). Round trip `Kernel.toRosen_toKlir` and faithfulness `Kernel.toRosen_injective` both hold. **Scored FAILED AS STATED, REPAIRED (ruled 2026-09-16).** Mapping 008 pre-registered the reading "the kernel's dependency Prop is read as 'F is defined on S'"; that reading has no relation in it to round-trip through, so the claim's own failure condition fired. The repair reads the dependency as Rosen's R_F, and its precondition is `Kernel.IsIndist`: the dependency must be an equivalence relation on the things, because R_F always is one. The memo's alternative failure mode ("needs the real line") did not occur — ℝ enters only as the codomain witness. What the cost *means* is stated as a theorem: every Rosen view projects to a symmetric relation (`RosenSystem.indist_symm`), so **a kernel with a one-way dependency has no Rosen view at all** (`rosen_no_view_of_asymmetric`). Rosen's pair carries no directed dependency among states; it carries indistinguishability and nothing else. Compare Bunge (a bond) and Mobus (irreflexivity): the Rosen view is the costliest of the four.

**Claim 3 — the Bunge witness (`Systems/Klir/RosenWitness.lean`).** Two faces of Bunge's bond, per `Bond.lean`'s note that bonding is symmetric while `ActsOn` is not. (i) *Directed bond, no view*: `directedPair`, a `ConcreteSystem Bool` with `false ▷ true` as its only structure, satisfies `bondage_nonempty`; `directedPair_has_no_rosen_view` says no `RosenSystem Bool` projects to its (T, R). The agent/patient direction has no home in (S, F). (ii) *Mutual bond, one reduced state*: `mutualKernel` (the full relation on `Bool`) has both a Bunge view and a Rosen view; `mutualRosen_observables` shows the Rosen view has exactly one observable, the constant 1, and `mutualRosen_indist_false_true` that the two bonded things are one reduced state. The bond survives only as loss of resolution: Rosen sees "bonded" as "inseparable by any observable", never as a relation between two things, and Bunge's a ≠ b is invisible. The memo's escape ("linkage relations among observables recover the bond") is closed for the directed face by `indist_symm` — no family of real-valued observables on S induces an asymmetric relation on S.

**Claim 4 — conjugacy (`Systems/Category/RosenConjugacy.lean`).** From p. 185: f′ is conjugate to f when a functor from the one-arrow category sends f to f′ and is *naturally equivalent* to the inclusion, "equivalent" fixed on the same page as "possess inverses"; p. 186, diagram (7.10.2), "where α, β are equivalences", and "Conjugacy imposes an equivalence relation on the set of all mappings in the category". `Conjugate f f′ := ∃ (α : A ≅ A′) (β : B ≅ B′), f ≫ β.hom = α.hom ≫ f′`. **`conjugate_iff_iso`: conjugacy is exactly isomorphism in Mathlib's `Arrow C`.** Rosen's p. 186 sentence follows (`conjugate_equivalence`). The groupoid is proper: `collapseSquare` (`Bool → Unit` over `𝟙 Unit`, left leg the collapse) is a morphism of C^→ and `square_not_conjugate` shows it is not a conjugacy (axiom-free). So the memo's prediction stands as stated: Rosen's modelling relation is the maximal subgroupoid of C^→, not C^→ itself, and the kernel's "system = morphism" reading puts Rosen's *objects* where the kernel puts its *morphisms*.

**Standard met.** `lake build` green (2285 jobs), zero sorry, zero custom axioms. All ten new theorems on propext/choice/Quot.sound except `square_not_conjugate`, which is axiom-free. `Systems.lean` imports both new files.
