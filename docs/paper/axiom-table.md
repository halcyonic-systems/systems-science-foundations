...
## Changelog
- **2026-07-23 — declared Dynamics record** (`Systems/Dynamics/Record.lean`): **zero new axioms — in fact axiom-free.** `#print axioms` on both example records reports no dependencies at all (not even `propext`/`Quot.sound`). Refines Mobus's opaque `τ` into the 5-field declaration record (support · carrier · kind · invariants · parameters); definitions + two enums + one `by decide`/`rfl` example. Conservative by parametricity — `τ` has no structural role (`Tuple.lean:48`), witnessed by a typechecking `MobusSystem … (Dynamics C P) …` example. Declaration layer only; the coalgebra semantics (endofunctor interpretation + openness) are bert-lenses#112, downstream. No change to the axiom count above. Detail: companion doc, "Downstream bindings".
