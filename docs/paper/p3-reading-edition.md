# Twelve principles: four primitives, two refinements, two stances, four theorems — the reading edition
...
**State of the result, 2026-09-04.**
1. Mobus's twelve principles are four ontological primitives (#1 Systemness, #4 Dynamics, #6 Evolution as `EvolutionE`, #8 Governance as `HomeostatD`), two structural refinements of Systemness (#3 Networks, #2 Hierarchy), two agential stances (#11 Understandability, #12 Improvability), and four theorems (#5, #7, #9, #10).
2. The count is computed, not asserted: every within-block ordered pair is either separated by a checked witness or realized by a checked derivation (`Systems/Principles/Matrix.lean`, `Hierarchy.lean`, `NonDegenerate.lean`, `EnvRelative.lean`).
3. One condition is stated: #8 is independent of #6 provided evolution's criterion is external (`sep_governanceD_evolutionE` / `sep_evolutionE_governanceD` against `evolvesByEnv_settle`), which is Mobus's own clause ("not resident in some mind").
4. The component–state bridge is adopted in its product form (`JointState`, `IsProductAggregate`) and environment things have a coordinate (`EnvState`); cross-block matrix cells are statable but not yet run.
5. "Twelve principles, eight axioms" (2026-06-09 to 2026-09-03) is the historical headline; the eight structures remain and are pairwise distinct, but as encoded #3 is #1 read twice, #2 presupposes #1, and #11/#12 are stances about the modeler.

**Thesis.** Mobus's twelve principles of systems science are not twelve independent commitments. Formalized in Lean 4, they resolve into four ontological primitives, two structural refinements of Systemness, two agential stances, and four theorems, with one stated condition (Governance is independent of Evolution provided evolution's criterion is external), and the derivation corrects several dependency claims Mobus states informally. The general point the reduction demonstrates is that a principles list is a theory, and theories have logical structure that prose cannot expose. Machine-checking a foundational vocabulary tells you which of its commitments are load-bearing, and the count itself moved once the independence claims were checked rather than argued.
...
Mobus lists twelve principles of systems science in prose, labelling two of them "corollaries" and grouping the last seven as the complex-adaptive-systems principles. Each principle was given a tractable formal core in Lean 4 with Mathlib and machine-checked, and the dependency structure was then computed: within each carrier block every ordered pair of structures is either separated by a checked witness or realized by a checked derivation (`Systems/Principles/Matrix.lean`). Four principles are ontological primitives (#1 Systemness; #4 Dynamics, `sep_dynamics_evolution`; #6 Evolution as `EvolutionE`, whose Red Queen 4-cycle evolves under no fixed order, `redQueen_evolutionE_not_evolution`; #8 Governance as `HomeostatD`, separated from #6 both ways by `sep_governanceD_evolutionE` and `sep_evolutionE_governanceD` provided evolution's criterion is external, since with fitness free every governing law is also environment-evolving, `evolvesByEnv_settle`), two are structural refinements of Systemness (#3 Networks is the bond graph read as a flow graph, `ConcreteSystem.toFlowNetwork`; #2 Hierarchy is decomposition plus within>between strength, `principle2_hierarchical`, presupposing #1 by construction and failing under uniform strength, `not_hierarchical_of_uniform`), two are agential stances about what an observer or designer does with a system (#11 Understandability; #12 Improvability in `DirectedAgent` / `DirectedUnderModel` form, bare `Improvement` being vacuous by `improved_iff_moving`), and four are theorems built from those structures with no new primitive (#5 Complexity from #1+#2, #7 Information, #9 Internal Models from #4, #10 Self-Models as the diagonal of #9). Three of Mobus's informal dependency claims fail under checking: #11 is not a corollary of #9, #12 is not a corollary of #6, and #5 is not an axiom; and the checking also corrected this program's own earlier headline, since "eight independent axioms" held only until the independence was computed rather than argued.
...
## §4 The eight axiom structures (four primitives, two refinements, two stances)

The eight structures below were, until 2026-09-03, presented as eight independent axioms. The within-block independence matrix then found three derivations among them (#1 ⇒ #3 in both directions, #8 ⇒ #6, bare #12 ⇔ non-degenerate #4) and one vacuity (#2 as Bunge's `ImmediateAncestor` asserts nothing), and the 2026-09-04 rebuilds resolved each: #2 was re-headlined as `Hierarchical` (Mobus Eq. 4.3 plus Simon's strength, so it presupposes #1 and can fail), #6 and #8 gained the environment coordinate (`EvolutionE`, `HomeostatD`) and separate both ways under an external fitness criterion, and #12 was restated as `DirectedAgent` / `DirectedUnderModel`. The regrouping that survives is: **primitives** #1, #4, #6, #8 (4.1, 4.4, 4.6, 4.5); **refinements of Systemness** #3, #2 (4.3, 4.2); **agential stances** #11, #12 (4.7, 4.8). The per-principle blocks are left in their original order and content, with the 2026-09-04 findings recorded in §9; where a block's structure name has been superseded (`ImmediateAncestor` → `Hierarchical`, `Evolution` → `EvolutionE`, `Homeostat` → `HomeostatD`, `Improvement` → `DirectedUnderModel`), the older structure still exists in the tree as the frozen or bare case.
...
| The component–state bridge | **Candidate built, machine-checked** (`Systems/Core/JointState.lean`, 2026-09-04) | `StateCarrier` (system + per-component `Q` + per-flow `K`), `JointState` = dependent product over components and flows, `LawfulDynamics` (Bunge's lawful subset), `Factors` / `IsProductAggregate` (aggregate = the law factors component-wise on the full product). **Separating theorem** `union_misses_neuron_aggregate`: on Bunge's own three-neuron aggregate (2³ = 8 states) any union of three two-point images has ≤ 6 points, so the existing union encoding (`State.lean:114-116`, `AggregateBridge.lean:93-94`) classifies Bunge's paradigm aggregate as a system. Companions: identity law factors (aggregate), cyclic shift does not (system). Profiles: propext/choice/Quot.sound for the cardinality theorem; propext/Quot.sound for the rest | **ADOPTED 2026-09-04** (Shingai): `IsProductAggregate` is the live criterion. Union reading retired, not deleted: `State.isAggregate` → `isAggregateUnion`, `StatefulComposite.stateAggregate` → `stateAggregateUnion`, old names kept as `@[deprecated]` aliases. `AggregateBridge.lean` rebuilt on the product: `bonded_yet_productAggregate` (three-neuron ring under the identity law), `bondFree_yet_productSystem` (two unbonded cells under the swap law), `productAggregate_criteria_independent`; Cor 5.14 kept as named hypotheses (`bondAggregate_iff_productAggregate_of_coupling`). Structural finding: `StateCarrier` sits on `ConcreteSystem`, whose Def 1.1 field makes every carrier bonded (`stateCarrier_organized`), so the bond-free direction lives on `ProductComposite` (CES data without Def 1.1). Memo §E questions 1–2 still go to Mobus/Bunge-reading. |
| Non-degeneracy conditions restore the separations and say what Mobus says | **Checked 2026-09-04, mixed** (`Systems/Principles/NonDegenerate.lean`) | Restored (4 of 7 cells, now dynamical not cardinality): #6 ⇏ #11, #8 ⇏ #11, #6 ⇏ #12, #8 ⇏ #12 via `UnderstoodNC` (non-constant model dynamics) on the one-tick collapse; `sep_governance_governanceNeg` shows negative feedback is a proper strengthening. **Not restored:** #8 ⇒ #6 survives any sharpening of Governance (`evolvesBy_of_governsNeg`); the separating target `EvolvesByEnv` needs an environment coordinate a homeostat on S lacks. Bare #12 ⇔ #4 survives: factoring `intervene` through the model is provably vacuous (`directedThroughModel_iff_directed`); only a *tracking* condition (`DirectedUnderModel`: the intervened law is still followed by the same model dynamics) bites, and it separates on the 4-cycle. **New content:** `EvolvesByEnv` (fitness relative to a stepping environment) is satisfiable by a Red Queen 4-cycle that is `EvolvesBy` under no fixed order (`redQueen_evolvesByEnv_not_evolvesBy`), and the Evolvable vacuity does not recur (`envEvolves_needs_env_change`, `envEvolves_needs_climb`). Fidelity: `GovernsNeg` matches "in opposition to the error"; `EvolvesByEnv` matches "long-term changes in their environments"; `UnderstoodNC` tightens "make predictions" only; nothing reaches Improvability's "better"/cost lines | Four things need a structure change, not a predicate: `Evolution` fixes one preorder (make it environment-indexed); `JointState` products over `composition` only (add environment things); bare `Improvement`'s free `intervene` (add a model-tracking field or retire bare #12 in favour of `DirectedAgent`); Governance has no disturbance input or band. **The honest count after this check:** Governance ⇒ Evolution stands as encoded; so state-side primitives are Dynamics, Evolution-with-environment, Understanding, and the agential Improvement, with Governance a refinement of Evolution unless the environment coordinate is added to homeostats. **Decision for Shingai + George.** |
| **The count, computed from the adopted encodings (2026-09-04, after A+B+C)** | Checked | Component block (`Hierarchy.lean`, `Matrix.lean`): #1 primitive; #3 = #1 read as a graph (D both ways); #2 = #1 + decomposition + strength (#2 ⇒ #1 by construction, #1 ⇏ #2 substantive). State block: #4 primitive; **#6 `EvolutionE`** (fitness indexed by a stepping environment; Red Queen is `EvolvableE` under no fixed order, `redQueen_evolutionE_not_evolution`; frozen case = old `Evolution`) primitive, `sep_dynamics_evolution`; **#8 `HomeostatD`** (disturbance input; `Robust` = Mobus 12-governance:307/309) separated from #6 both ways on `Bool × Bool` (`sep_governanceD_evolutionE`, `sep_evolutionE_governanceD`) **provided fitness is pinned to an external order**; with the fitness family free, every governing law is also environment-evolving (`evolvesByEnv_settle`: choose "set point is fittest"), so #8 is a primitive exactly because Mobus's #6 criterion is external ("not resident in some mind") and #8's is internal. #11 and #12 agential (12 in `DirectedAgent`/`DirectedUnderModel` form; bare `Improvement` retired as vacuous). Theorems unchanged: #5, #7, #9, #10 | **Twelve principles = four ontological primitives (Systemness, Dynamics, Evolution, Governance) + two structural refinements of Systemness (Networks, Hierarchy) + two agential stances (Understandability, Improvability) + four theorems (Complexity, Information, Internal Models, Self-Models).** Governance's independence carries one stated condition: the criterion of evolution is external. Say exactly this in the abstract |
...
**Pairwise independence, honestly.** *Decided state, 2026-09-04.* Within each carrier block every ordered pair is either separated by a checked witness or realized by a checked derivation (`Systems/Principles/Matrix.lean`, 76 declarations; `Hierarchy.lean`; `NonDegenerate.lean`; `EnvRelative.lean`); the ledger rows above record which cells were derivations and what was rebuilt. The count that results is four primitives, two refinements, two stances, four theorems, with #8's independence from #6 conditional on an external fitness criterion. Cross-block cells (component × state) are statable through `EnvState` / `JointState` and have not been run. The two tables that follow are the pre-matrix witness inventory of 2026-09-03 and are kept as the record of what existed before the matrix. At that time eight axioms gave 28 unordered pairs and 56 directed non-derivability claims, and the source contained separating witnesses for exactly the following directed claims between axioms:
...
As of 2026-09-03 every other directed pair (for example #1 ⇏ #4, #4 ⇏ #8, #12 ⇏ #6, #2 ⇏ #3, and all pairs involving #1 or #2 as target) had no witness and was supported only by the absence of a derivation. The matrix closed that gap within blocks on 2026-09-03 and found that several of those unwitnessed pairs were in fact derivations (#1 ⇒ #3, #8 ⇒ #6, bare #12 ⇔ non-degenerate #4); the `Systems/Principles.lean` header now states the decided count and names the cross-block cells as the remaining unrun part.

**Recommended wording for the title claim** (decided 2026-09-04). "Eight independent axioms" is withdrawn: it was false as stated for three pairs. The honest headline is *twelve principles: four primitives, two refinements, two stances, four theorems*, and the abstract should say exactly this: *Mobus's twelve principles, formalized in Lean 4, resolve into four ontological primitives, two structural refinements of Systemness, two agential stances, and four theorems; Governance's independence from Evolution holds provided evolution's criterion is external.* "Axiom" may still be used for the eight structures in the sense of §3 (a structure not produced by the others' structures), but the paper must then say that three of them are refinements or stances rather than independents. The one condition is part of the result, not a caveat: Mobus's own clause that evolution's criterion is "not resident in some mind" is what makes Governance a primitive. Both the old outline's abstract and the vault spine's title are revised to match; "twelve principles, eight axioms" remains in git as the pre-recount headline.
...
**Ruling (2026-09-04).** Product bridge adopted; see the §9 row above for what changed. The "Union bridge" row in this table describes the state before the ruling and is left as written.
